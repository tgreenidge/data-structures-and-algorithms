...
const getHouses = (arr) => {
  let houses = [];
  Object.keys(arr).forEach( char => {
    houses.push(arr[char]['house']);
  });
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
  let result;
  arr.forEach(char => {
    if(Object.values(char)[0] === character) {
      result = (Object.values(char)[2]).length > 0;
    }
  });
...
const hasChildrenEntries = (arr, character) => {
  let result;
  arr.forEach(char => {
    if(Object.entries(char)[0][1] === character) {
      result = Object.entries(char)[2][1].length;
    }
  });
  return result > 0;
};
...
const totalCharacters = (arr) => {
  ...
  arr.forEach(item => {
    totalNumCharacters += item['children'].length + 1;
    if(item['spouse']) {
      totalNumCharacters += 1;
    }
  });

  return totalNumCharacters;
};
