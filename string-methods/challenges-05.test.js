...
const listFoods = (recipe) => {
  ...
  recipe['ingredients'].forEach(ingredient => {
    let ingredientNoQuantity = ingredient.slice(ingredient.indexOf(' ') + 1);
    result.push(ingredientNoQuantity.slice(ingredientNoQuantity.indexOf(' ') + 1));
  });
  return result;
};
...
const removeEvenValues = (arr) => {
  ...
  for (let i = 0; i <= arr.length - nextOdd + 1; i++){
    arr.pop();
  }
...
const removeVowels = (str) => {
  let result = '';
  for (let i = 0; i < str.length; i++){
    if(!['e','a', 'i', 'o', 'u'].includes(str[i].toLowerCase())){
      result += str[i];
    }
  }
